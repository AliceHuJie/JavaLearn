...
## 集群原理  
 ...
 根据取模结果与每个小集群的槽位范围。
  ...
  redis 客户端服务端都有一个各小集群的映射表（如上图），得出hash_slot后就会像对应的机器ip发出请求。
  ...
  超时时间是为了防止网络抖动时，心跳探测短暂收不到误以为master挂掉，又重新选了新的master等情况。 需要根据网络情况进行集群超时时间的合理设置。  
